...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (data.slides.size() > 0) {
            ...
            Slide secondSlide = result.get(1);
            Slide lastSlide = result.getLast();
            Slide preLastSlide = result.get(result.size() - 2);

            for (Slide slide : data.slides) {

                //check start of snake
                {
                    int score = calculateScore(slide, firstSlide, secondSlide);
                ...
                {
                    int score = calculateScore(preLastSlide, lastSlide, slide);
                    if (score > highestScore) {
                        highestScore = score;
                        highestSlide = slide;
                        isStart = false;
    ...
    public static int calculateScore(Slide slide1, Slide slide2, Slide slide3) {
        return calculateScore(slide1, slide2) + calculateScore(slide2, slide3);
    }
